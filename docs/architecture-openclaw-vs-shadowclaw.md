...
| **시스템 프롬프트** | OpenClaw와 동일 섹션: Identity → Project Context → Output format → Tooling → **Tool Call Style** → **Safety** → **Skills (Mandatory)** → **Memory**(hasMemoryTools 시) → **Workspace**(workspaceDir 시) → **Silent Replies** → **Heartbeats** → **Runtime** → forceSkill. PromptMode: full / minimal / none. |
| **도구 호출** | **ReAct**: 프롬프트로 "JSON만 출력. action: call|answer, skill, args". `extractJson()` 파싱 후 `executeTool()` → observation 문자열 배열로 누적. |
| **메시지** | `completeWithContext({ systemPrompt, messages })`: **messages[]**는 history + 현재 턴 user. **assistant 턴**은 히스토리만 있고, **도구 결과는 현재 턴 user 메시지 안 "Observations" 블록**으로만 넣음. |
| **세션** | SQLite (`sessions`, `messages`). Turn: role, content. **tool_calls_json** 등으로 도구 호출 저장 가능하나, LLM에는 observations 텍스트로만 전달. |
...
|------|----------|------------|------|
| **Bootstrap 파일 수** | 9종 (AGENTS, SOUL, TOOLS, IDENTITY, USER, HEARTBEAT, BOOTSTRAP, MEMORY, memory) | 4종 (SOUL, USER, MEMORY, IDENTITY) | ShadowClaw는 AGENTS, TOOLS, HEARTBEAT, BOOTSTRAP 미사용. |
...
| 항목 | OpenClaw | ShadowClaw | 비고 |
|------|----------|------------|------|
| **PromptMode** | full / minimal / none | full / minimal / none | 동일. react.ts는 full 사용. |
| **Safety 섹션** | 있음 (목표 제한, 안전 우선 등) | 동일 문구 | |
| **Skills (mandatory)** | "한 스킬 선택 → read로 SKILL.md 읽고 따르라" | "Tooling 도구 사용, 한 번에 하나" | SKILL.md read 없음; ReAct용 이름+요약+params. |
| **Memory Recall** | memory_search / memory_get 사용 규칙 | hasMemoryTools 시 Memory 섹션 포함 | MEMORY.md는 contextFiles로만; 도구 없으면 섹션 생략. |
| **Workspace 경로 안내** | workspaceDir(·sandbox) 명시 | workspaceDir 전달 시 Workspace 섹션 | react.ts에서 resolveWorkspaceDir() 전달. |
...
| 항목 | OpenClaw | ShadowClaw | 비고 |
|------|----------|------------|------|
| **도구 호출** | 네이티브 API `tools` + `tool_calls` | ReAct JSON 파싱 (action, skill, args) | 근본적 차이. |
| **도구 스키마** | API `tools` 배열로 전달 | 시스템/현재 턴 텍스트(skillParamsBlock) | 네이티브 전환 시 API로 이전 필요. |
| **도구 결과** | 메시지 배열에 role: tool / tool_use 블록 | 현재 턴 user 메시지 안 "Observations" 텍스트 | OpenClaw는 구조화된 메시지로 재호출. |
| **트랜스크립트 위생** | sanitizeSessionHistory, repair 등 | 없음 | |
| **히스토리 형식** | messages[] (user/assistant/tool) | turnsToMessages로 role+content만, 12턴·2000자 제한 | 도구 결과가 메시지가 아님. |

### 3.4 세션·스토어

| 항목 | OpenClaw | ShadowClaw | 비고 |
|------|----------|------------|------|
| **세션 저장** | Pi SessionManager + 세션 파일 | SQLite (sessions, messages) | 저장 방식만 다름. |
| **히스토리 역할** | API와 동일한 메시지 형식 | Turn[] → messages[] 변환, 도구 호출은 별도 필드 | 네이티브 전환 시 메시지 형식 통일 필요. |
...
1. **Bootstrap**: 파일 4종만 사용, 캐시·boundary-file·missing 주입·hook·세션별 필터 없음.
2. **시스템 프롬프트**: Safety, Skills(Mandatory), Memory 규칙, Workspace 안내, Silent/Heartbeat, Runtime, Tool Call Style 없음. PromptMode 없음.
3. **도구**: ReAct JSON 방식. 네이티브 tool calling·API tools·도구 결과 메시지 반영·트랜스크립트 위생 없음.
4. **메시지**: 히스토리는 messages[]로 넘기지만, 도구 결과는 Observations 텍스트로만 포함. role: tool / tool_use 없음.
5. **Skills**: SKILL.md 기반이 아니라 레지스트리+프롬프트 주입.
6. **워크스페이스**: 온보딩·BOOTSTRAP 생성·resolveUserPath 없음.
...
이 문서는 `docs/modularization.md`, `docs/plan-native-tool-calling.md`와 함께 참고하면 된다.
